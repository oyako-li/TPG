...
from _tpg.configuration.conf_agent import ConfAgent, ConfAgent1, ConfAgent3, ConfAgent2
from _tpg.configuration.conf_team import ConfTeam, ConfTeam1, ConfTeam3, ConfTeam2
from _tpg.configuration.conf_learner import ConfLearner, ConfLearner1, ConfLearner3, ConfLearner2
from _tpg.configuration.conf_action_object import ConfActionObject, ConfActionObject3, ConfActionObject1
from _tpg.configuration.conf_program import ConfProgram, ConfProgram1, ConfProgram3, ConfProgram2
...
def configure3(trainer, Trainer, Agent, Team, Learner, ActionObject, Program,
        doMemory, memType, doReal, operationSet, traversal):
    ...
    mutateParamKeys = ["generation", "maxTeamSize", "pLrnDel", "pLrnAdd", "pLrnMut",
        "pProgMut", "pActMut", "pActAtom", "pInstDel", "pInstAdd", "pInstSwp", "pInstMut",
        "nDestinations", "inputSize", "initMaxProgSize",
        "rampantGen", "rampantMin", "rampantMax", "idCountTeam", "idCountLearner", "idCountProgram"]
    mutateParamVals = [trainer.generation, trainer.maxTeamSize, trainer.pLrnDel, trainer.pLrnAdd, trainer.pLrnMut,
        trainer.pProgMut, trainer.pActMut, trainer.pActAtom, trainer.pInstDel, trainer.pInstAdd, trainer.pInstSwp, trainer.pInstMut,
        trainer.nRegisters, trainer.inputSize, trainer.initMaxProgSize,
        trainer.rampancy[0], trainer.rampancy[1], trainer.rampancy[2], 0, 0, 0]
    ...
    # before doing any special configuration, set all methods to defaults
    _configureDefaults3(trainer, Trainer, Agent, Team, Learner, ActionObject, Program)

    # configure Program execution stuff, affected by memory and operations set
    _configureProgram3(trainer, Learner, Program, actVarKeys, actVarVals, mutateParamKeys, mutateParamVals, doMemory, memType, operationSet)

    # configure stuff for using real valued actions
    if doReal:  _configureRealAction3(trainer, ActionObject, mutateParamKeys, mutateParamVals, doMemory)

    # do learner traversal
    if traversal == "learner": _configureLearnerTraversal3(trainer, Agent, Team, actVarKeys, actVarVals)
...
For each class in TPG, sets the functions to their defaults.
"""
def _configureDefaults3(trainer, Trainer, Agent, Team, Learner, ActionObject, Program):
    # set trainer functions
    # TODO: add learner configurable

    # set agent functions
    Agent.__init__ = ConfAgent3.init_def
    Agent.act = ConfAgent3.act_def
    Agent.reward = ConfAgent3.reward_def
    Agent.taskDone = ConfAgent3.taskDone_def
    Agent.saveToFile = ConfAgent3.saveToFile_def

    # set team functions
    Team.__init__ = ConfTeam3.init_def
    Team.act = ConfTeam3.act_def
    Team.addLearner = ConfTeam3.addLearner_def
    Team.removeLearner = ConfTeam3.removeLearner_def
    Team.removeLearners = ConfTeam3.removeLearners_def
    Team.numAtomicActions = ConfTeam3.numAtomicActions_def
    Team.mutate = ConfTeam3.mutate_def
    Team.clone = ConfTeam3.clone_def

    # set learner functions
    Learner.__init__ = ConfLearner3.init_def
    Learner.bid = ConfLearner3.bid_def
    Learner.getAction = ConfLearner3.getAction_def
    Learner.getActionTeam = ConfLearner3.getActionTeam_def
    Learner.isActionAtomic = ConfLearner3.isActionAtomic_def
    Learner.mutate = ConfLearner3.mutate_def
    Learner.clone = ConfLearner3.clone_def


    # set action object functions
    ActionObject.__init__ = ConfActionObject3.init_def
    ActionObject.getAction = ConfActionObject3.getAction_def
    ActionObject.isAtomic = ConfActionObject3.isAtomic_def
    ActionObject.mutate = ConfActionObject3.mutate_def
    # ActionObject.actions = trainer.actionCodes


    # set program functions
    Program.__init__ = ConfProgram3.init_def
    Program.execute = ConfProgram3.execute_def
    Program.mutate = ConfProgram3.mutate_def
    Program.memWriteProbFunc = ConfProgram3.memWriteProb_def
...
Decides the operations and functions to be used in program execution.
"""
def _configureProgram3(trainer, Learner, Program, actVarKeys, actVarVals,
        mutateParamKeys, mutateParamVals, doMemory, memType, operationSet):
    # change functions as needed
    if doMemory:
        # default (reduced) or full operation set
        if operationSet == "def":
            Program.execute = ConfProgram3.execute_mem
            trainer.functionsDict["Program"]["execute"] = "mem"
            trainer.nOperations = 7
            trainer.operations = ["ADD", "SUB", "MULT", "DIV", "NEG", "MEM_READ", "MEM_WRITE"]
        elif operationSet == "full":
            Program.execute = ConfProgram3.execute_mem_full
            trainer.functionsDict["Program"]["execute"] = "mem_full"
            trainer.nOperations = 10
            trainer.operations = ["ADD", "SUB", "MULT", "DIV", "NEG", "COS", "LOG", "EXP", "MEM_READ", "MEM_WRITE"]
        elif operationSet == "robo":
            Program.execute = ConfProgram3.execute_mem_robo
            trainer.functionsDict["Program"]["execute"] = "mem_robo"
            trainer.nOperations = 8
            trainer.operations = ["ADD", "SUB", "MULT", "DIV", "NEG", "COS", "MEM_READ", "MEM_WRITE"]
        elif operationSet == "custom":
            Program.execute = ConfProgram3.execute_mem_custom
            trainer.functionsDict["Program"]["execute"] = "mem_custom"
            trainer.nOperations = 18
            trainer.operations = ["ADD", "SUB", "MULT", "DIV", "POW", "NEG", "INV_NEG", "SIN", "COS", "TANH", "LN", "SQRT", "EXP", "POWY2", "POWY3", "ABS", "MEM_READ", "MEM_WRITE"]
        ...
        # select appropriate memory write function
        if memType == "cauchy1":
            Program.memWriteProbFunc = ConfProgram3.memWriteProb_cauchy1
            trainer.functionsDict["Program"]["memWriteProbFunc"] = "cauchy1"
        elif memType == "cauchyHalf":
            Program.memWriteProbFunc = ConfProgram3.memWriteProb_cauchyHalf
            trainer.functionsDict["Program"]["memWriteProbFunc"] = "cauchyHalf"
        else:
            Program.memWriteProbFunc = ConfProgram3.memWriteProb_def
            trainer.functionsDict["Program"]["memWriteProbFunc"] = "def"
        ...
        # change bid function to accomodate additional parameters needed for memory
        Learner.bid = ConfLearner3.bid_mem
        trainer.functionsDict["Learner"]["bid"] = "mem"
        ...
        if operationSet == "def":
            ...
            Program.execute = ConfProgram3.execute_full
            trainer.functionsDict["Program"]["execute"] = "full"
            trainer.nOperations = 8
            trainer.operations = ["ADD", "SUB", "MULT", "DIV", "NEG", "COS", "LOG", "EXP"]
        elif operationSet == "robo":
            Program.execute = ConfProgram3.execute_robo
            trainer.functionsDict["Program"]["execute"] = "robo"
            trainer.nOperations = 6
            trainer.operations = ["ADD", "SUB", "MULT", "DIV", "NEG", "COS"]
        elif operationSet == "custom":
            Program.execute = ConfProgram3.execute_custom
            trainer.functionsDict["Program"]["execute"] = "custom"
            trainer.nOperations = 16
            trainer.operations = ["ADD", "SUB", "MULT", "DIV", "POW", "NEG", "INV_NEG", "SIN", "COS", "TANH", "LN", "SQRT", "EXP", "POWY2", "POWY3", "ABS"]
...
Make the appropriate changes needed to be able to use real actions.
"""
def _configureRealAction3(trainer, ActionObject, mutateParamKeys, mutateParamVals, doMemory):
    # change functions as needed
    ActionObject.__init__ = ConfActionObject3.init_real
    trainer.functionsDict["ActionObject"]["init"] = "real"
    ActionObject.getAction = ConfActionObject3.getAction_real
    trainer.functionsDict["ActionObject"]["getAction"] = "real"
    if doMemory:
        ActionObject.getRealAction = ConfActionObject3.getRealAction_real_mem
        trainer.functionsDict["ActionObject"]["getRealAction"] = "real_mem"
    else:
        ActionObject.getRealAction = ConfActionObject3.getRealAction_real
        trainer.functionsDict["ActionObject"]["getRealAction"] = "real"
    ActionObject.mutate = ConfActionObject3.mutate_real
    trainer.functionsDict["ActionObject"]["mutate"] = "real"
...
Switch to learner traversal.
"""
def _configureLearnerTraversal3(trainer, Agent, Team, actVarKeys, actVarVals):
    Team.act = ConfTeam3.act_learnerTrav
    trainer.functionsDict["Team"]["act"] = "learnerTrav"
