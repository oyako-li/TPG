...
class Team1:
    ...
    def _mutation_mutate(self, probability, mutateParams, teams):
        ...
        for learner in original_learners:
            if flip(probability):
                ...
                #print("Team {} creating learner".format(self.id))
                # Create a new new learner 
                newLearner = Learner1(mutateParams, learner.program, learner.actionObj, len(learner.registers), learner.id)
                new_learners.append(newLearner)
                # Add the mutated learner to our learners
                # Must add before mutate so that the new learner has this team in its inTeams
                self.addLearner(newLearner)
    ...
    def __eq__(self, o: object) -> bool:
        ...
        # Object must be instance of Team
        if not isinstance(o, Team1):    return False
    ...
    def configFunctions(cls, functionsDict):
        ...
        if functionsDict["init"] == "def":
            cls.__init__ = ConfTeam1.init_def

        if functionsDict["act"] == "def":
            cls.act = ConfTeam1.act_def
        elif functionsDict["act"] == "learnerTrav":
            cls.act = ConfTeam1.act_learnerTrav

        if functionsDict["addLearner"] == "def":
            cls.addLearner = ConfTeam1.addLearner_def

        if functionsDict["removeLearner"] == "def":
            cls.removeLearner = ConfTeam1.removeLearner_def

        if functionsDict["removeLearners"] == "def":
            cls.removeLearners = ConfTeam1.removeLearners_def

        if functionsDict["numAtomicActions"] == "def":
            cls.numAtomicActions = ConfTeam1.numAtomicActions_def

        if functionsDict["mutate"] == "def":
            cls.mutate = ConfTeam1.mutate_def
...
class Team2:
    ...
    def _mutation_delete(self, probability):
            ...
            while flip(probability) and len(self.learners) > 2: # must have >= 2 learners
                ...
                if self.numAtomicMemories() > 1:
                    ...
                    learner = random.choice(valid_choices)
                # if learner.isMemoryAtomic(): MemoryObject.memories[learner.memoryObj.memoryCode]-=1
    ...
    def __eq__(self, o: object) -> bool:
        ...
        # Object must be instance of Team
        if not isinstance(o, Team2):    return False
    ...
    @classmethod
    def configFunctions(cls, functionsDict):
        from _tpg.configuration.conf_team import ConfTeam2
        ...
        elif functionsDict["image"] == "learnerTrav":
        ...
        if functionsDict["removeLearner"] == "def":
            cls.removeLearner = ConfTeam2.removeLearner_def
...
class Team3:
    ...
    def _mutation_mutate(self, probability, mutateParams, teams):
        ...
        for learner in original_learners:
            if flip(probability):
                ...
                #print("Team {} creating learner".format(self.id))
                # Create a new new learner 
                newLearner = Learner3(mutateParams, learner.program, learner.actionObj, len(learner.registers), learner.id)
                new_learners.append(newLearner)
                # Add the mutated learner to our learners
                # Must add before mutate so that the new learner has this team in its inTeams
                self.addLearner(newLearner)
    ...
    def __eq__(self, o: object) -> bool:
        ...
        # Object must be instance of Team
        if not isinstance(o, Team3):    return False
    ...
    @classmethod
    def configFunctions(cls, functionsDict):
        from _tpg.configuration.conf_team import ConfTeam3

        if functionsDict["init"] == "def":
            cls.__init__ = ConfTeam3.init_def

        if functionsDict["act"] == "def":
            cls.act = ConfTeam3.act_def
        elif functionsDict["act"] == "learnerTrav":
            cls.act = ConfTeam3.act_learnerTrav

        if functionsDict["addLearner"] == "def":
            cls.addLearner = ConfTeam3.addLearner_def

        if functionsDict["removeLearner"] == "def":
            cls.removeLearner = ConfTeam3.removeLearner_def

        if functionsDict["removeLearners"] == "def":
            cls.removeLearners = ConfTeam3.removeLearners_def

        if functionsDict["numAtomicActions"] == "def":
            cls.numAtomicActions = ConfTeam3.numAtomicActions_def

        if functionsDict["mutate"] == "def":
            cls.mutate = ConfTeam3.mutate_def
        
        if functionsDict["clone"] == "def":
            cls.clone = ConfTeam3.clone_def
